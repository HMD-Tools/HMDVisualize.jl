...
function bond_pbc(s::AbstractSystem, color_func::Function)
    ...
    for edge in edges(topology(s))
        ...
        println(diff)
        if diff == [0, 0, 0]
            # 周期境界との交差無し，bondの始点と終点のみ
            cross_points = [
                (progress=0.0, cross_dim=0),
                (progress=1.0, cross_dim=0)
            ]
        else
            if diff[1] != 0
                # x軸方向の周期境界とbondが交差
                ...
                push!(cross_points, (progress=_t, cross_dim=1))
            end
            if diff[2] != 0
                β = diff[2]
                (_t, α, γ) = (-p1 + β*b + origin) \ [bond_vector -a -c]
                push!(cross_points, (progress=_t, cross_dim=2))
            end
            if diff[3] != 0
                γ = diff[3]
                (_t, α, β) = (-p1 + γ*c + origin) \ [bond_vector -a -b]
                push!(cross_points, (progress=_t, cross_dim=3))
            end
        end
        ...
        center = (progress=0.5, cross_dim=0)
        ic = searchsortedfirst(cross_points, center; by=x->x.progress)
        insert!(cross_points, ic, center)

        #if length(cross_points) > 2
        ...
        bond_type = _bond_type(s, atom_id1, atom_id2)
        pbc_shift = zeros(3)
        for i in 2:length(cross_points)
            t, t_prev = cross_points[i].progress, cross_points[i-1].progress
            cl = t < 0.5 ? color1 : color2
            p  = p1 + t*bond_vector + pbc_shift
            add_bdata!(
                bonds[bond_type],
                p, -(t-t_prev)*bond_vector, cl
            )
            dim = cross_points[i].cross_dim
            if dim != 0
